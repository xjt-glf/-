<template>
	<div class="box">
		<el-card class="box-card" style="height:auto">
			...
			<div class="filter">
				<span>人员类型</span>
				<el-select v-model="selectedPersonsTypes" clearable placeholder="请选择类型">
				<el-option
				...
				<span>人员状态</span>
				<el-select v-model="selectedPersonsStatus" clearable placeholder="请选择状态">
				<el-option
					v-for="item in personsStatus"
					:key="item.value"
					:label="item.label"
					:value="item.value">
				</el-option>
				...
				<el-button type="primary" style="margin-left:1%" @click="getPerson">查询</el-button>
				<el-button type="primary" style="margin-left:26%" @click="addDevice">添加人员</el-button>
			...
			<el-container style="padding: 10px 25px 0 25px">
				<el-main>
					...
					<el-table-column prop="id" label="人员id" min-width="15%" align="center"
						header-align="center"></el-table-column>
					
					<el-table-column prop="type" label="人员类型" min-width="15%" align="center"
						header-align="center"></el-table-column>

					<el-table-column prop="status" label="人员状态" min-width="15%" align="center"
						header-align="center"></el-table-column>
			...
			<div style="color:white" v-if="firstShowDialog">
				...
				>
					<div class="">
						...
						<div class='c'>人员id: {{this.onSeeDevice.id}}</div>
						...
						<el-button type="primary" style="margin-top: 10px;" @click="updatePeople">确认修改</el-button>
					</div>
				</el-dialog>
			</div>
			<div style="color:white" v-if="showAddDeviceDialog">
				...
				>
					<div class="">
						...
						<!-- <div id=''>{{this.onSeeDevice.loc}}</div>   -->
						<el-button type="primary" style="margin-top: 10px;" @click="addPeople">提交表单</el-button>
					</div>
				</el-dialog>
			</div>
	...
	export default {
	  data() {
	    return {
			...
			time:"",
			formatTime:'' ,
			selectedPersonsTypes:'',
			selectedPersonsStatus:'',
			persons:[],
			selectedPersons:[],
			showDialog:false,
			onSeeDevice:null,
	  ...
	  methods: {
		getPersonTypeList(){
			...
			this.$axios.request(options).then(function (response) {
				...
				response.data.forEach(item=>{
					...
					that.personTypes.push(devicesTypeItem);
				})
		...
		handleCurrentChange(val) {
			...
			// console.log(this.selectedpersonTypes)
		...
		getPerson(){
			...
			if(this.selectedPersonsStatus==''){
				this.selectedPersonsStatus=null;
			}
			let that=this;
			this.$axios({
                method: 'GET',
                url: 'http://47.94.10.119:20008/SituationDisplay/getPersonPosition',
                params: {page: '1',
				 		pagenum: '-1',
						type:this.selectedPersonsTypes,
						status:this.selectedPersonsStatus,
					}
			}).then(response => {
				this.persons=[];
				// console.log(response.data);
				...
				response.data.forEach(item =>{
					...
					if(item["latitude"]==null){
						...
						person["loc"]=item["longitude"]+','+item["latitude"];
					}
			...
			this.selectedAddType=null;
			this.closeAddDeviceDialog=false;
		},
		onSee(row){ 
		...
		onDelete(row){
			...
			const options = {
				method: 'GET',
				url: 'http://47.94.10.119:20008//PeopleMange/delPeople',
				params: {id: row.id}
			};
			let that=this;
			this.$axios.request(options).then(function (response) {
		...
		updatePeople(){
			// console.log("111111");
			// console.log(this.position?this.position.split(',')[0]:null);
			// console.log(this.position?this.position.split(',')[1]:null);

			const options = {
			method: 'GET',
			url: 'http://47.94.10.119:20008/PeopleMange/modPeople',
			params: {
		...
		addPeople(){
			...
			const options = {
				method: 'GET',
				url: 'http://47.94.10.119:20008//PeopleMange/addPeople',
				params: {
					// id: '11', 
					info: this.infoAdd, 
			...
			this.$axios.request(options).then(function (response) {
				...
				if(response.status=='200'){
					...
					that.infoAdd=null;
					that.positionAdd=null;
					that.selectedAddStatus=null;
					that.getPerson();
				}else{
					that.$message.error("请求异常");
				}
